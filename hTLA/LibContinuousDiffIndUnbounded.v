...
Section parametric.
  ...
  : CActionProp :=
    fun p =>
      exists is_derivable,
        solves_diffeqs _ p diffeqs is_derivable.
  ...
  Proof.
    ...
    unfold starts, discretely, Continuous, before, through. simpl.
  ...
  Definition Dt_R {L} {IL : ILogicOps L} {E : EmbedOp Prop L} (g : R -> R) (P : (R -> R) -> L) : L :=
    Exists is_derivable : derivable g,
    Forall g' : R -> R,
      (Forall t', embed (derive g is_derivable t' = g' t')) -->>
      P g'.

  Definition Dt (g : tlaState -> R) (P : (R -> R) -> CActionProp) : CActionProp :=
    fun p =>
      Dt_R (fun x => g (p x)) P p.
  ...
  Definition throughCt (P : CStateProp) : CActionProp :=
    fun p => forall t', P t' (p t').

  Definition equal (f : R -> R) (r : R -> R -> Prop) (g : R -> R)
  : CStateVal Prop :=
    fun t st => r (f t) (g t).
  ...
  Proof.
    ...
    specialize (H (derive (fun x => f (p x)) x) (fun _ => eq_refl)).
    do 2 red in H; simpl in H. forward_reason.
    specialize (H _ (fun _ => eq_refl)).
    unfold throughCt, equal in H.
    pose (pf := derivable_minus _ _ x x0).
    generalize (@MVT.null_derivative_1 (fun x => f (p x) - g (p x))%R pf).
    intro Hmvt.
    cut_prems Hmvt.
    { clear - H0 Hmvt.
      red in Hmvt.
      ...
      erewrite Hmvt.
      apply RIneq.Rminus_diag_eq. eapply H0. }
    { intros.
      unfold pf.
      (** opacity **)
      erewrite pr_nu.
      generalize (derive_pt_minus (fun x => f (p x)) (fun x => g (p x)) x1 (x x1) (x0 x1)).
      intro.
      etransitivity. eapply H1.
      specialize (H x1).
      unfold derive in H. rewrite H.
...
*)
  ...
  Proof.
    ...
    { simpl. split.
      { clear IHcp.
        ...
        intros. red. intros.
        rewrite <- (H0 t'); clear H0.
        destruct a. simpl in *.
        eapply H. }
      { eapply IHcp.
        instantiate (1:=fun v pf => x v (or_intror _ pf)).
        clear - H. revert H.
        unfold solves_diffeqs. simpl.
        intros. eapply H. } }
  Qed.

  Definition is_derivative_R (f' : R -> R) (f : R -> R) : Prop :=
    exists is_derivable : derivable f,
      forall t',
        f' t' = derive f is_derivable t'.
  ...
  Proof.
    ...
    clear - H.
    rewrite (FunctionalExtensionality.functional_extensionality _ _ H).
    clear. tauto.
  Qed.
