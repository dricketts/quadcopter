...
Module Type Trace.
  Parameter time : Type.
  Parameter time_plus : time -> time -> time.

  Parameter stream : Type -> Type.
  Parameter action : Type -> Type.

  Parameter hd : forall {T}, stream T -> T.

  Parameter next_time : forall {T}, stream T -> time.

  Parameter after_time : forall {T}, time -> stream T -> stream T -> Prop.

  Parameter FirstStep : forall {T}, stream T -> action T -> Prop.

  Parameter Discretely : forall {T}, action T -> T -> stream T -> Prop.

  Axiom Reflexive_skips_to : forall {T}, Reflexive (@skips_to T).
  Axiom Transitive_skips_to : forall {T}, Transitive (@skips_to T).
End Trace.

Module TraceLogic (Tr : Trace).

Existing Instance Tr.Reflexive_skips_to.
Existing Instance Tr.Transitive_skips_to.
...
Section parametic.
  ...
  Definition ActionVal (T : Type) : Type :=
    tlaState -> Tr.action tlaState -> T.
  Definition TraceVal (T : Type) :=
    Tr.stream tlaState -> T.
  ...
  Global Instance ILogicOps_DActionProp : ILogicOps DActionProp :=
    @ILInsts.ILFun_Ops _ _ _.
  Global Instance ILogicOps_CActionProp : ILogicOps CActionProp :=
    @ILInsts.ILFun_Ops _ _ _.
...
*)
  ...
  Definition always (P : TraceProp) : TraceProp :=
    fun s =>
      forall s', Tr.skips_to s s' -> P s'.

  Definition eventually (P : TraceProp) : TraceProp :=
    fun s =>
      exists s', Tr.skips_to s s' /\ P s'.

  Definition discretely (P : DActionProp) : ActionProp :=
    fun start step =>
      exists st' tr', Tr.Discretely step st' tr' /\ P start st'.
...
*)
  ...
  Definition through (P : TraceProp) : TraceProp :=
    (** Note: This could be enriched **)
    fun tr =>
      exists act,
        Tr.FirstStep tr act /\
        (exists st' tr', Tr.Discretely act st' tr' /\
                         P tr').
