...
Section parametric.
  ...
  : CActionProp :=
    fun p t =>
      exists is_derivable,
        solves_diffeqs _ p diffeqs t is_derivable.
...
*)
  ...
  Proof.
     ...
     **)
(*
    generalize (fun pr H => @MVT.derive_increasing_interv_var 0 t (fun x => f (p x) - g (p x))%R pr H4 H 0 t' H5 H6 H3).
    pose (pf := (fun x1 pf =>
                   let pf' : (0 <= x1 <= t)%R := open_closed _ _ _ pf in
                   derivable_pt_minus _ _ _ (x _ pf') (x0 _ pf'))).
    intro Hmvt.
...
*)
  ...
  : forall cp x y,
      ContinuousC cp
      |-- Exists f' : tlaState, AllOf (List.map (fun de => nowC (o de.(de_lhs) f' 
