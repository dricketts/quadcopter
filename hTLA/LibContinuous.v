...
Section parametric.
   ...
   **)
  Definition deriv_state (obs : list (Var tlaState R)) (t : R)
             (f f' : R -> tlaState)
  : Prop :=
  ...
  (* Prop expressing that f is a solution to diffeqs in
   [0,r]. *)
  Definition is_solution (f : R -> tlaState)
             (diffeqs : list DiffEq) (r : R) :=
    exists is_derivable,
  ...
  Definition Continuous (cp:list DiffEq) (P : StateProp tlaState) : DActionProp tlaState :=
    let xs := List.map (fun x => @existT _ (Var tlaState) R x.(de_lhs)) cp in
    Exists r : R,
    Exists f : R -> tlaState,
               (embed (r > 0)%R)
          //\\ (embed (forall t, (0 <= t <= r)%R -> P (f t)))
          //\\ (embed (is_solution f cp r))
